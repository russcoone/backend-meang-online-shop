...
class ShopProductsService extends resolver_operations_service_1.default {
    constructor(root, variables, context) {
        super(root, variables, context);
        this.collection = constants_1.COLLECTION.SHOP_PRODUCT;
    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                updateList.map((item) => __awaiter(this, void 0, void 0, function* () {
                    console.log(item);
                    const itemDetails = yield (0, db_operations_1.findOneElement)(this.getDb(), constants_1.COLLECTION.SHOP_PRODUCT, { id: +item.id });
                    if (item.increment < 0 && ((item.increment + itemDetails.stock) < 0)) {
                        item.increment = -itemDetails.stock;
                    }
                    yield (0, db_operations_1.manageStockUpdate)(this.getDb(), constants_1.COLLECTION.SHOP_PRODUCT, { id: +item.id }, { stock: item.increment });
                    itemDetails.stock += item.increment;
                    pubsub.publish(constants_1.SUBSCRIPTIONS_EVENT.UPDATE_STOCK_PRODUCT, { selectProductStockUpdate: itemDetails });
                }));
                return true;
            }
            catch (e) {
